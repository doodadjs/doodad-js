	...
	modules['Doodad'] = {
		...
		create: function create(root, /*optional*/_options, _shared) {
			...
			tools.complete(_shared.Natives, {
				...
				arraySpliceApply: _shared.Natives.functionBindApply(global.Array.prototype.splice),
			...
			_shared.catchExceptionHook = function catchExceptionHook(ex, /*optional*/obj, /*optional*/attr) {
				if (ex.stack) {
					...
					_shared.Natives.functionCallCall(types.Error.prototype.parse, ex);
			...
			types.ADD('_implements', root.DD_DOC(
				...
				, function _implements(obj, type) {
					...
					const impls = types.getAttribute((!isType && ((cls[_shared.ModifiersSymbol] || 0) & doodad.ClassModifiers.Expandable) ? obj : cls), _shared.ImplementsSymbol, _shared.SECRET);
			...
			types.ADD('getImplements', root.DD_DOC(
				...
				, function getImplements(obj) {
					...
					const isType = types.isType(obj),
						cls = (isType ? obj : types.getType(obj));
					return types.getAttribute((!isType && ((cls[_shared.ModifiersSymbol] || 0) & doodad.ClassModifiers.Expandable) ? obj : cls), _shared.ImplementsSymbol, _shared.SECRET);
				}));

			types.ADD('getMethodScope', root.DD_DOC(
				//! REPLACE_IF(IS_UNSET('debug'), "null")
					{
						author: "Claude Petit",
						revision: 0,
						...
						returns: 'integer',
						description: "Returns the scope of an object method. Returns 'null' on invalid object or method.",
					}
				//! END_REPLACE()
				, function getMethodScope(obj, fn) {
					if (!types.isLike(obj, __Internal__.CLASS_OR_INTERFACE)) {
						return null;
					};
					const isFunction = types.isFunction(fn);
					const name = (isFunction ? fn[_shared.NameSymbol] : (types.isSymbol(fn) ? fn : types.toString(fn)));
					if (!name) {
						return null;
					};
					const isType = types.isType(obj);
					const values = types.getAttributes(obj, [name, _shared.AttributesSymbol], _shared.SECRET);
					const attributes = values[_shared.AttributesSymbol];
					...
					if (isFunction && (fn !== values[name])) {
						return null;
					};
					return attribute[_shared.ScopeSymbol];
				}));
			...
			types.ADD('isMethod', root.DD_DOC(
				...
				//! END_REPLACE()
				, function isMethod(obj, fn) {
					const scope = types.getMethodScope(obj, fn);
					return !types.isNothing(scope);
				}));
			...
			types.ADD('isImplemented', root.DD_DOC(
				...
				, function isImplemented(obj, name) {
					...
					const attrs = types.getAttribute(obj, _shared.AttributesSymbol, _shared.SECRET);
					...
					const method = types.getAttribute(obj, name, _shared.SECRET);
			...
			doodad.ADD('Callback', root.DD_DOC(
				...
				, function Callback(/*optional*/obj, fn, /*optional*/bubbleError, /*optional*/args, /*optional*/secret) {
					...
					const callback = function callbackHandler(/*paramarray*/...params) {
						...
						try {
							...
							if (callBubble && !ex.bubble) {
								...
								try {
									doodad.trapException(ex, obj, null);
								} catch(o) {
									types.DEBUGGER();
								};
							};
					...
					types.setJsAttribute(callback, _shared.CallbackSymbol, doodad.Callback, {});
					types.setJsAttribute(callback, _shared.BoundObjectSymbol, orgObj, {});
					types.setJsAttribute(callback, _shared.OriginalValueSymbol, fn, {});
					callback.lastError = null;
					return callback;
				}));

			types.setJsAttribute(doodad.Callback, _shared.BaseSymbol, types.Callback, {});
			...
			doodad.ADD('AsyncCallback', root.DD_DOC(
				...
				, function AsyncCallback(/*optional*/obj, fn, /*optional*/bubbleError, /*optional*/args, /*optional*/secret) {
					...
					types.setJsAttribute(callback, _shared.CallbackSymbol, doodad.AsyncCallback, {});
			...
			types.setJsAttribute(doodad.AsyncCallback, _shared.BaseSymbol, doodad.Callback, {});

			//==================================
			// Inside
			//==================================

			__Internal__.isInside = function isInside(obj, /*optional*/state) {
				if (types.isNothing(obj)) {
					if (state) {
						return types.isNothing(state[0]) && types.isNothing(state[1]);
					} else {
						return types.isNothing(__Internal__.currentInstance) && types.isNothing(__Internal__.currentType);
					}
				} else {
			...
			__Internal__.setInside = function setInside(obj, secret, /*optional*/preserve) {
				const type = types.getType(obj);
				if (!type || !__Internal__.isInside(obj)) {
					if (type && (types.isClass(type) || types.isInterfaceClass(type)) && !types.isNothing(_shared.SECRET) && (secret !== _shared.SECRET)) {
						throw new types.ValueError("Invalid secret.");
					};
			...
			__Internal__.preserveInside = function preserveInside() {
				return [__Internal__.currentInstance, __Internal__.currentType];
			};

			__Internal__.restoreInside = function restoreInside(state) {
			...
			__Internal__.makeInside = function makeInside(/*optional*/obj, fn, /*optional*/secret) {
				...
				root.DD_ASSERT && root.DD_ASSERT((!types.isNothing(obj) && types.isBindable(fn)) || (types.isNothing(obj) && types.isFunction(fn)), "Invalid function.");
				const scope = types.getMethodScope(obj, fn);
				if (scope === doodad.Scopes.Public) {
					// Public methods doesn't need to be made inside.
					return fn;
				};
				...
				_insider = types.setPrototypeOf(_insider, doodad.InsiderFunction);
				types.setJsAttribute(_insider, _shared.BoundObjectSymbol, obj, {});
				types.setJsAttribute(_insider, _shared.OriginalValueSymbol, fn, {});
				return _insider;
			};

			__Internal__.insideNew = function insideNew(...args) {
			...
			_shared.makeInside = root.DD_DOC(
				...
				, function makeInside(/*optional*/obj, fn, /*optional*/secret) {
					...
					if (types.isClass(type) || types.isInterfaceClass(type)) {
						...
						return __Internal__.makeInside(null, fn);
					};
				});

			//_shared.getCurrentType = function getCurrentType() {
			//	return __Internal__.currentType;
			//};

			//_shared.getCurrentInstance = function getCurrentInstance() {
			//	return __Internal__.currentInstance;
			//};
			...
			_shared.invoke = function invoke(/*optional*/obj, fn, /*optional*/args, /*optional*/secret) {
				...
				const scope = types.getMethodScope(obj, fn);
				if (scope === doodad.Scopes.Public) {
					// Public methods doesn't need to be invoked.
			...
			types.ADD('getAttribute', root.DD_DOC(
				//! REPLACE_IF(IS_UNSET('debug'), "null")
					{
						author: "Claude Petit",
						revision: 2,
						params: {
							...
							attr: {
								type: 'string,symbol',
								optional: false,
								description: "Attribute name or symbol.",
							},
							secret: {
								type: 'any',
								optional: true,
						...
						returns: 'any',
						description: "Get an attribute value from a Doodad object like being inside that object.",
					}
				//! END_REPLACE()
				, function getAttribute(obj, attr, /*optional*/secret) {
					const type = types.getType(obj);
					const needsInside = type && (types.isClass(type) || types.isInterfaceClass(type));
					if (needsInside) {
						const oldInside = __Internal__.setInside(obj, secret, true);
						try {
							...
							if (types.hasIn(storage, attr)) {
								...
								return types.getJsAttribute(obj, attr);
							}
						} finally {
							if (oldInside) {
								__Internal__.restoreInside(oldInside);
			...
			types.ADD('getAttributes', root.DD_DOC(
				//! REPLACE_IF(IS_UNSET('debug'), "null")
					{
						author: "Claude Petit",
						revision: 2,
						params: {
							...
							attrs: {
								...
								optional: false,
								description: "Attribute names or symbols.",
							},
							secret: {
								type: 'any',
								optional: true,
						...
						returns: 'object',
						description: "Get attributes from a Doodad object like being inside that object.",
					}
				//! END_REPLACE()
				, function getAttributes(obj, attrs, /*optional*/secret) {
					const type = types.getType(obj);
					const needsInside = type && (types.isClass(type) || types.isInterfaceClass(type));
					if (needsInside) {
						const oldInside = __Internal__.setInside(obj, secret, true);
						...
						return types.getJsAttributes(obj, attrs);
					}
				}));

			types.ADD('setAttribute', root.DD_DOC(
				//! REPLACE_IF(IS_UNSET('debug'), "null")
					{
						author: "Claude Petit",
						revision: 2,
						...
						returns: 'object',
						description: "Sets the attribute value of a Doodad object like being inside that object.",
					}
				//! END_REPLACE()
				, function setAttribute(obj, attr, value, /*optional*/secret) {
					const type = types.getType(obj);
					const needsInside = type && (types.isClass(type) || types.isInterfaceClass(type));
					if (needsInside) {
						const oldInside = __Internal__.setInside(obj, secret, true);
						try {
							...
							if (types.hasIn(storage, attr)) {
								...
								return types.setJsAttribute(obj, attr, value);
							}
						} finally {
							if (oldInside) {
								__Internal__.restoreInside(oldInside);
			...
			types.ADD('setAttributes', root.DD_DOC(
				//! REPLACE_IF(IS_UNSET('debug'), "null")
					{
						author: "Claude Petit",
						revision: 2,
						...
						returns: 'object',
						description: "Sets the attribute values of a Doodad object like being inside that object.",
					}
				//! END_REPLACE()
				, function setAttributes(obj, values, /*optional*/secret) {
					const type = types.getType(obj);
					const needsInside = type && (types.isClass(type) || types.isInterfaceClass(type));
					if (needsInside) {
						const oldInside = __Internal__.setInside(obj, secret, true);
							...
							const loopKeys = function _loopKeys(keys) {
								...
								for (let i = 0; i < keysLen; i++) {
									...
									if (types.hasIn(storage, key)) {
										...
										types.setJsAttribute(obj, key, values[key]);
									};
								};
							};
						...
						return types.setJsAttributes(obj, values);
					}
				}));
			...
			_shared.DESTROY = function DESTROY(obj) {
				if (types._implements(obj, mixIns.Creatable)) {
					const destroyed = types.getAttribute(obj, _shared.DestroyedSymbol, _shared.SECRET);
			...
			_shared.REGISTER = root.DD_DOC(
				...
				, function REGISTER(type, args, protect) {
					...
					if (isType || isErrorType) {
						...
						//	bind: type.bind,
						//};
						//types.setJsAttributes(type, values, {all: false, ignoreWhenReadOnly: true}, _shared.SECRET);
					...
					if (!types.get(newType, 'DD_FULL_NAME')) {
						types.setJsAttributes(newType, {
							DD_PARENT: this,
							DD_NAME: name,
							DD_FULL_NAME: fullName,
						}, {all: false}, _shared.SECRET);
					};
					...
					if (!isPrivate) {
						...
						const entry = new entryType(root, null, newType, {protect});
						entry.init();
						...
						overrideOptions: root.DD_DOC(
							...
							, function overrideOptions(options, newOptions, /*optional*/replace) {
								if (replace) {
									...
									options.isProto = ((newOptions.isProto === null) && (this.isProto === null) ? null : (!!newOptions.isProto === true) && (this.isProto === true));
								};
								return options;
							}),
			...
			doodad.ADD('CallerFunction', types.freezeObject(function CallerFunction() {}));
			doodad.ADD('DispatchFunction', types.freezeObject(function DispatchFunction() {}));

			doodad.ADD('AttributeGetter', types.freezeObject(function AttributeGetter() {}));
			doodad.ADD('AttributeSetter', types.freezeObject(function AttributeSetter() {}));
			...
			root.DD_DOC(
				...
				, extenders.REGISTER([], extenders.Extender.$inherit({
					...
					getterTemplate: root.DD_DOC(
						//! REPLACE_IF(IS_UNSET('debug'), "null")
							{
								...
								params: {
									...
									tmpStorage: {
										type: 'object',
										optional: false,
										description: "Temporary storage object.",
									},
								},
								returns: 'function',
						...
						//! END_REPLACE()
						, function getterTemplate(attr, attribute, forType, tmpStorage) {
							const extender = this;
							const cache = {
								obj: null,
								storage: null,
							...
							const getter = function getter() {
								if (__Internal__.hasScopes) {
									if (extender.enableScopes) {
										if (!__Internal__.isInside(this)) {
											const result = types.getAttributes(this, [_shared.CurrentDispatchSymbol, _shared.CurrentCallerIndexSymbol], _shared.SECRET);
											const dispatch = result[_shared.CurrentDispatchSymbol],
												caller = result[_shared.CurrentCallerIndexSymbol];
											if (attribute[_shared.ScopeSymbol] === doodad.Scopes.Private) {
												if (!dispatch || (dispatch[_shared.CallersSymbol][caller][_shared.PrototypeSymbol] !== attribute[_shared.PrototypeSymbol])) {
													throw new types.Error("Attribute '~0~' of '~1~' is private.", [attr, types.getTypeName(this) || __Internal__.ANONYMOUS]);
												};
											} else if (attribute[_shared.ScopeSymbol] === doodad.Scopes.Protected) {
												if (!dispatch) {
								...
								if ((attr === _shared.AttributesStorageSymbol) && !cache.obj) {
									cache.obj = this;
									cache.storage = tmpStorage;
									tmpStorage = null; // free memory
							...
							types.setJsAttribute(getter, _shared.CallerSymbol, doodad.AttributeGetter, {});
							return getter;
						}),
					...
					setterTemplate: root.DD_DOC(
						...
						//! END_REPLACE()
						, function setterTemplate(attr, attribute, forType, tmpStorage) {
							const extender = this;
							...
							const setter = function setter(value) {
								...
								if (__Internal__.hasPolicies && extender.isReadOnly) {
									...
									if ((attr === _shared.AttributesStorageSymbol) && !cache.obj) {
										value[_shared.AttributesStorageSymbol] = value;
										cache.obj = this;
										tmpStorage[_shared.AttributesStorageSymbol] = value;
										cache.storage = value;
										tmpStorage = null; // free memory
									} else {
										if (cache.obj !== this) {
							...
							types.setJsAttribute(setter, _shared.CallerSymbol, doodad.AttributeSetter, {});
							return setter;
						}),
					...
					init: root.DD_DOC(
						...
						, function init(attr, attributes, forType, attribute, value, generator, isProto, existingAttributes) {
							...
							if (this.__isFromStorage(attribute)) {
								...
								generator.code.add(
									"types.setJsAttribute(" + generator.objId + ", " + attrId + ", " + valueId + ", " + descId + ")"
								);
							};
						}),
			...
			root.DD_DOC(
				...
				, extenders.REGISTER([], extenders.Attribute.$inherit({
					...
					init: types.SUPER(function init(attr, attributes, forType, attribute, value, generator, isProto, existingAttributes) {
						if (this.cloneOnInit) {
							if (root.DD_ASSERT) {
								root.DD_ASSERT((this.isProto === false), "When an attribute is cloned at initialisation, it must be 'isProto === false'.");
								root.DD_ASSERT(types.isClonable(value), "The value must be clonable.");
							};
			...
			root.DD_DOC(
				...
				, extenders.REGISTER([], extenders.ClonedAttribute.$inherit({
					...
					isOutside: types.READ_ONLY(false),

					methodProperties: types.READ_ONLY(null),  // will be set in constructor
					...
					callerTemplate: root.DD_DOC(
						...
						, function callerTemplate(attr, sourceAttribute, destAttribute) {
							...
							types.setJsAttributes(fn, values, {all: false, ignoreWhenReadOnly: true});

							const _caller = function caller(/*paramarray*/...params) {
								//const type = types.getType(this);

								const result = types.getAttributes(this, [_shared.CurrentDispatchSymbol, _shared.CurrentCallerIndexSymbol, '_super'], _shared.SECRET),
								...
								try {
									...
									types.setAttribute(this, '_super', _super, _shared.SECRET);
									types.setAttribute(this, _shared.CurrentCallerIndexSymbol, currentCaller, _shared.SECRET);

									const callFn = (extender.byReference ? fn : types.unbox(_caller[_shared.PrototypeSymbol][attr]));

									_caller[_shared.CalledSymbol] = true;

									let retVal = _shared.Natives.functionApplyCall(callFn, this, params);
									...
									types.setAttribute(this, '_super', oldSuper, _shared.SECRET);
									if (!types.isNothing(currentCaller)) {
										types.setAttribute(this, _shared.CurrentCallerIndexSymbol, currentCaller - 1, _shared.SECRET);
									};
							...
							types.setJsAttribute(_caller, _shared.CallerSymbol, doodad.CallerFunction, {});
					...
					validateDispatchResult: function validateDispatchResult(result, attr, async, attribute, obj, secret) {
						if (async) {
							...
							if (__Internal__.hasPolicies) {
								...
								if (validator) {
									result = result.then(function(val) {
										if (!_shared.Natives.functionCallCall(validator, obj, val)) {
							...
							if (__Internal__.hasPolicies) {
								...
								if (validator && !_shared.Natives.functionCallCall(validator, obj, result)) {
									if (attr === 'toString') {
					...
					handleDispatchError: function handleDispatchError(ex, attr, obj) {
						...
						const attributes = types.getAttribute(obj, _shared.AttributesSymbol, _shared.SECRET);
						const notReentrantMap = __Internal__.notReentrantMap.get(obj);
						let emitted = false;
						if (!ex.bubble) {
							if ((types.isClass(type) || types.isInterfaceClass(type)) && obj._implements(mixIns.Events)) {
								...
								if (obj._implements(mixIns.Creatable)) {
									destroyed = types.getAttribute(obj, _shared.DestroyedSymbol, _shared.SECRET);
								};
								if (destroyed === false) { // NOTE: Can be 'null' for "not created".
									...
									if (errorEvent && (attr !== errorEvent) && (!notReentrantMap || !notReentrantMap.get(errorEvent))) {
										...
										const onError = types.getAttribute(obj, errorEvent, _shared.SECRET);
					...
					dispatchTemplate: root.DD_DOC(
						...
						, function dispatchTemplate(attr, attribute, callers) {
							...
							const _dispatch = function dispatch(/*paramarray*/...params) {
								const type = types.getType(this),
								...
								if (!canBeDestroyed) {
									...
									if (types._implements(this, mixIns.Creatable)) {
										const destroyed = types.getAttribute(this, _shared.DestroyedSymbol, _shared.SECRET);
								...
								const result = types.getAttributes(this, [_shared.CurrentDispatchSymbol, _shared.CurrentCallerIndexSymbol/*, _shared.AttributesSymbol*/], _shared.SECRET),
								...
								//if (host) {
								//	const result = types.getAttributes(host, [_shared.CurrentDispatchSymbol, _shared.CurrentCallerIndexSymbol], _shared.SECRET);
								...
								const oldInside = (extender.isOutside ? __Internal__.preserveInside() : __Internal__.setInside(this, _shared.SECRET, true));
								...
								try {
									...
									types.setAttributes(this, values, _shared.SECRET);

									//if (host) {
									//	types.setAttributes(host, values, _shared.SECRET);
									//};

									//_dispatch[_shared.SuperAsyncSymbol] = false;

									if (notReentrant) {
										notReentrantMap.set(attr, oldDispatch || true);
									};

									caller[_shared.CalledSymbol] = false;

									retVal = _shared.Natives.functionApplyCall(caller, this, params);
							...
							types.setJsAttribute(_dispatch, _shared.CallerSymbol, doodad.DispatchFunction, {});
					...
					createDispatch: root.DD_DOC(
						...
						, function createDispatch(attr, obj, attribute, callers) {
							...
							types.setJsAttribute(dispatch, _shared.NameSymbol, attr, {});
							types.setJsAttribute(dispatch, _shared.CallersSymbol, callers, {});
							...
							types.setJsAttribute(dispatch, _shared.ModifiersSymbol, modifiers, {});
					...
					_new: types.SUPER(function _new(/*optional*/options) {
						...
						types.setAttributes(this, {
							...
							isOutside: types.get(options, 'isOutside', this.isOutside),

							methodProperties: types.freezeObject([_shared.NameSymbol, _shared.CallersSymbol]),
						});
					}),
					...
					getCacheName: types.SUPER(function getCacheName(/*optional*/options) {
						return this._super(options) +
								...
								',' + (types.get(options, 'isOutside', this.isOutside) ? '1' : '0');
					}),

					overrideOptions: types.SUPER(function overrideOptions(options, newOptions, /*optional*/replace) {
						options = this._super(options, newOptions, replace);
						if (replace) {
							tools.fill(['bindMethod', 'notReentrant', 'byReference', 'isExternal', 'isOutside'], options, this, newOptions);
						} else {
							...
							options.isOutside = !!newOptions.isOutside || this.isOutside;
						};
						return options;
					}),

					applyMethodProperties: function applyMethodProperties(dest, source) {
						tools.forEach(this.methodProperties, function(key) {
					...
					postExtend: root.DD_DOC(
						...
						, types.SUPER(function postExtend(attr, destAttributes, destAttribute) {
							...
							let callFirstLen = callers.length;
							...
							destAttribute[_shared.CallFirstLengthSymbol] = callFirstLen;
					...
					init: types.SUPER(function init(attr, attributes, forType, attribute, value, generator, isProto, existingAttributes) {
						...
						//const hasValue = !types.isNothing(value);
			...
			root.DD_DOC(
				...
				, extenders.REGISTER([], extenders.Method.$inherit({
					...
					callerTemplate: function callerTemplate(attr, sourceAttribute, destAttribute) {
						...
						types.setJsAttribute(fn, _shared.CallerSymbol, doodad.CallerFunction, {});

						const _caller = tools.nullObject();

						types.setJsAttribute(_caller, _shared.FunctionSymbol, fn, {});
						...
						, function jsCallerTemplate(attr, fn, /*optional*/_super) {
							...
							if (this.dontSetSuper) {
								_caller = function caller(/*paramarray*/...params) {
									const oldSuper = types.getAttribute(this, '_super', _shared.SECRET);
									try {
										return fnApply(this, params);
									} finally {
										types.setAttribute(this, '_super', oldSuper, _shared.SECRET);
					...
					dispatchTemplate: function dispatchTemplate(attr, attribute, callers) {
						...
						for (let i = callersLen - 1; i >= 0; i--) {
							...
							types.setJsAttribute(_super, _shared.PrototypeSymbol, caller[_shared.PrototypeSymbol], {});
						};
						...
						types.setJsAttribute(_super, _shared.CallerSymbol, doodad.DispatchFunction, {});
					...
					extend: function extend(attr, source, sourceProto, destAttributes, forType, sourceAttribute, destAttribute, sourceIsProto, proto, protoName) {
						sourceAttribute = _shared.Natives.functionCallCall(extenders.ClonedAttribute.extend, this, attr, source, sourceProto, destAttributes, forType, sourceAttribute, destAttribute, sourceIsProto, proto, protoName);
					...
					postExtend: root.DD_DOC(
						...
						, function postExtend(attr, destAttributes, destAttribute) {
							//destAttribute = _shared.Natives.functionCallCall(extenders.ClonedAttribute.postExtend, this, attr, destAttributes, destAttribute) || destAttribute;
			...
			root.DD_DOC(
				...
				, extenders.REGISTER([], extenders.Method.$inherit({
					...
					isProto: types.READ_ONLY(false),

					extend: types.SUPER(function extend(attr, source, sourceProto, destAttributes, forType, sourceAttribute, destAttribute, sourceIsProto, proto, protoName) {
						sourceAttribute = _shared.Natives.functionCallCall(extenders.Attribute.extend, this, attr, source, sourceProto, destAttributes, forType, sourceAttribute, destAttribute, sourceIsProto, proto, protoName);
					...
					init: function init(attr, attributes, forType, attribute, value, generator, isProto, existingAttributes) {
						...
						//const valueId = ((attr === _shared.AttributesStorageSymbol) ? generator.storageId : generator.vars.add(value));
						const valueId = generator.vars.add(value);
						const descriptor = new generator.DynamicValue("tools.extend({}, " + valueId + ")");
					...
					remove: types.SUPER(function remove(attr, obj, storage, forType, attribute) {
						_shared.Natives.functionCallCall(extenders.ClonedAttribute.remove, this, attr, obj, storage, forType, attribute);
					}),
				})));
			...
			doodad.ADD('BASE', root.DD_DOC(
				...
				, function BASE(cls) {
					...
					types.setAttribute(cls, _shared.ModifiersSymbol, (cls[_shared.ModifiersSymbol] || 0) | doodad.ClassModifiers.Base, _shared.SECRET);
					return cls;
				}));
			...
			doodad.ADD('TRAIT', doodad.ADD('MIX_IN', root.DD_DOC(
				...
				, function MIX_IN(cls) {
					...
					types.setAttribute(cls, _shared.ModifiersSymbol, ((cls[_shared.ModifiersSymbol] || 0) & ~doodad.ClassModifiers.Interface) | doodad.ClassModifiers.MixIn, _shared.SECRET);
					return cls;
				})));
			...
			doodad.ADD('INTERFACE', root.DD_DOC(
				...
				, function INTERFACE(cls) {
					...
					types.setAttribute(cls, _shared.ModifiersSymbol, ((cls[_shared.ModifiersSymbol] || 0) & ~doodad.ClassModifiers.MixIn) | doodad.ClassModifiers.Interface, _shared.SECRET);
					return cls;
				}));
			...
			doodad.ADD('SEALED', root.DD_DOC(
				...
				, function SEALED(cls) {
					...
					types.setAttribute(cls, _shared.ModifiersSymbol, (cls[_shared.ModifiersSymbol] || 0) | doodad.ClassModifiers.Sealed, _shared.SECRET);
					return cls;
				}));
			...
			doodad.ADD('STATIC', root.DD_DOC(
				...
				, function STATIC(cls) {
					...
					types.setAttribute(cls, _shared.ModifiersSymbol, (cls[_shared.ModifiersSymbol] || 0) | doodad.ClassModifiers.Static, _shared.SECRET);
					return cls;
				}));
			...
			doodad.ADD('ISOLATED', root.DD_DOC(
				...
				, function ISOLATED(cls) {
					...
					types.setAttribute(cls, _shared.ModifiersSymbol, (cls[_shared.ModifiersSymbol] || 0) | doodad.ClassModifiers.Isolated, _shared.SECRET);
					return cls;
				}));
			...
			doodad.ADD('EXPANDABLE', root.DD_DOC(
				...
				, function EXPANDABLE(cls) {
					...
					types.setAttribute(cls, _shared.ModifiersSymbol, (cls[_shared.ModifiersSymbol] || 0) | doodad.ClassModifiers.Expandable, _shared.SECRET);
					return cls;
				}));
				...
				//! REPLACE_IF(IS_UNSET('debug'), "null")
					{
						...
						params: {
							type: {
								...
								optional: false,
								description: "Class or interface type.",
							},
							value: {
								type: 'AttributeBox,any',
								optional: true,
			...
			doodad.ADD('OUTSIDE', root.DD_DOC(
				//! REPLACE_IF(IS_UNSET('debug'), "null")
					{
						author: "Claude Petit",
						revision: 0,
						...
						returns: 'AttributeBox,Extender',
						description: "Specifies a method that runs outside its object, but with the correct 'this'.",
					}
				//! END_REPLACE()
				, function OUTSIDE(/*optional*/fn) {
					return doodad.BIND(doodad.OPTIONS({
						isOutside: true,
					}, doodad.METHOD(fn)));
				}));
			...
			__Internal__.defaultAttributesExpandable = types.freezeObject(tools.nullObject({
				[_shared.AttributesSymbol]: doodad.PRIVATE(doodad.READ_ONLY(doodad.NOT_INHERITED(doodad.PERSISTENT(doodad.PRE_EXTEND(doodad.TYPE(doodad.INSTANCE(doodad.ATTRIBUTE(null, extenders.ClonedAttribute, {isEnumerable: false, cloneOnInit: true, isProto: false})))))))),
				[_shared.ImplementsSymbol]: doodad.PRIVATE(doodad.READ_ONLY(doodad.NOT_INHERITED(doodad.PERSISTENT(doodad.PRE_EXTEND(doodad.TYPE(doodad.INSTANCE(doodad.ATTRIBUTE(null, extenders.ClonedAttribute, {isEnumerable: false, cloneOnInit: true, isProto: false})))))))),
				[_shared.ToInitializeSymbol]: doodad.PRIVATE(doodad.READ_ONLY(doodad.NOT_INHERITED(doodad.PERSISTENT(doodad.PRE_EXTEND(doodad.TYPE(doodad.INSTANCE(doodad.ATTRIBUTE(null, extenders.ClonedAttribute, {isEnumerable: false, cloneOnInit: true, isProto: false})))))))),
				[_shared.ToExtendLaterSymbol]: doodad.PRIVATE(doodad.READ_ONLY(doodad.NOT_INHERITED(doodad.PERSISTENT(doodad.PRE_EXTEND(doodad.TYPE(doodad.INSTANCE(doodad.ATTRIBUTE(null, extenders.ClonedAttribute, {isEnumerable: false, cloneOnInit: true, isProto: false})))))))),
			}));
			...
			__Internal__.switchToExpandable = function switchToExpandable(attributes) {
				tools.forEach(__Internal__.defaultAttributesExpandable, function(attribute, attr) {
					...
					root.DD_ASSERT && root.DD_ASSERT(oldAttribute, "Trying to add a new attribute that is not supported.");
...
*/
			...
			__Internal__.preExtendAttribute = function preExtendAttribute(attr, base, baseIsClass, baseAttributes, source, sourceProto, sourceAttributes, destAttributes, baseIsProto, sourceIsProto, forType, _isolated, extendedAttributes, toInitialize, proto, protoName, _implements, toExtendLater, sources, isExtendLater) {
				...
				let retval = undefined; // ESLINT "consistent-return"
				...
				return retval; // ESLINT "consistent-return"
			};
			...
			__Internal__.extendSource = function extendSource(base, baseTypeAttributes, baseInstanceAttributes, source, sourceTypeAttributes, sourceInstanceAttributes, destTypeAttributes, destInstanceAttributes, baseType, baseIsType, baseIsClass, sourceIsType, sourceIsClass, _isolated, typeExtendedAttributes, instanceExtendedAttributes, typeToInitialize, instanceToInitialize, proto, protoName, _implements, typeToExtendLater, instanceToExtendLater, sources) {
				...
				if (sourceIsClass) {
					// doodad-js Class
					sourceTypeProto = types.getAttribute(source, _shared.PrototypeSymbol, _shared.SECRET);
					...
					sourceIsolated.forEach(function(data, _interface) {
						if (!types.isString(_interface)) {
							...
							if (!_isolated.has(_interface) && (!uuid || !_isolated.has(uuid))) {
								...
								const impls = types.getAttribute(_interface, _shared.ImplementsSymbol, _shared.SECRET).values();
				...
				if (baseType && !types.baseof(sourceType, baseType)) { // prevents cyclic extend
					...
					if (!isImplemented) {
						...
						if (types.isIsolated(source) && !types.isIsolated(base)) {
							...
							const sourceData = types.getAttributes(source, [_shared.ImplementsSymbol], _shared.SECRET);
							...
							const baseTypeData = types.getAttributes(baseType, [_shared.ImplementsSymbol, _shared.AttributesSymbol, _shared.IsolatedSymbol, _shared.ToInitializeSymbol, _shared.ToExtendLaterSymbol], _shared.SECRET);
							const baseInstanceData = types.getAttributes(baseType.prototype, [_shared.AttributesSymbol, _shared.ToInitializeSymbol, _shared.ToExtendLaterSymbol], _shared.SECRET);

							baseTypeAttributes = baseTypeData[_shared.AttributesSymbol];
							baseInstanceAttributes = baseInstanceData[_shared.AttributesSymbol];

							__Internal__.addImplements(_implements, _isolated, typeToInitialize, instanceToInitialize, typeToExtendLater, instanceToExtendLater, base, baseTypeAttributes, baseInstanceAttributes, baseTypeData[_shared.ImplementsSymbol], baseTypeData[_shared.IsolatedSymbol], baseIsType, baseTypeData[_shared.ToInitializeSymbol], baseInstanceData[_shared.ToInitializeSymbol], baseTypeData[_shared.ToExtendLaterSymbol], baseInstanceData[_shared.ToExtendLaterSymbol]);
						...
						if (sourceIsClass) {
							const sourceTypeData = types.getAttributes(sourceType, [_shared.ImplementsSymbol, _shared.AttributesSymbol, _shared.IsolatedSymbol, _shared.ToInitializeSymbol, _shared.ToExtendLaterSymbol], _shared.SECRET);
							const sourceInstanceData = types.getAttributes(sourceType.prototype, [_shared.AttributesSymbol, _shared.ToInitializeSymbol, _shared.ToExtendLaterSymbol], _shared.SECRET);
			...
			_shared.createGenerator = function createGenerator() {
				const generator = {
					...
					define: function define(attrId, descId) {
						...
						generator.code.add(generator.propsId + "[" + attrId + "] = " + descId);
					},
					build: function() {
			...
			__Internal__.createType = function createType(base, baseType, baseIsType, proto, protoName, protoUUID, typeStorage, instanceStorage, destTypeAttributes, destInstanceAttributes, typeExtendedAttributes, instanceExtendedAttributes, typeToInitialize, instanceToInitialize, _isolated, _implements, modifiers, typeToExtendLater, instanceToExtendLater, existingAttributes) {
				...
				if (baseIsType) {
					...
					types.setJsAttribute(proto, _shared.TypeSymbol, newType, {});
					...
					types.setJsAttributes(newType, newTypeValues, {configurable: true});
					...
					types.setJsAttributes(newProto, newProtoValues, {configurable: true});
					...
					if (base._implements(mixIns.Creatable)) {
						types.setAttribute(base, _shared.DestroyedSymbol, null, _shared.SECRET);
					};
			...
			__Internal__.$extend = function $extend(/*paramarray*/...args) {
				...
				if (baseIsType) {
					...
					if (baseIsClass) {
						...
						const baseTypeData = types.getAttributes(baseType, [_shared.AttributesSymbol, _shared.ModifiersSymbol], _shared.SECRET);
						baseTypeAttributes = baseTypeData[_shared.AttributesSymbol];
						...
						const baseInstanceData = types.getAttributes(baseType.prototype, [_shared.AttributesSymbol], _shared.SECRET);
						baseInstanceAttributes = baseInstanceData[_shared.AttributesSymbol];
					} else {
						baseTypeAttributes = baseType;
						baseInstanceAttributes = baseType.prototype;
					...
					const baseTypeData = types.getAttributes(baseType, [_shared.ModifiersSymbol, _shared.AttributesStorageSymbol, _shared.ToExtendLaterSymbol], _shared.SECRET);
					...
					const baseData = types.getAttributes(base, [_shared.AttributesSymbol, _shared.ImplementsSymbol, _shared.IsolatedSymbol, _shared.AttributesStorageSymbol, _shared.ToExtendLaterSymbol], _shared.SECRET);
			...
			__Internal__._delete = types.SUPER(
				function _delete() {
					...
					_shared.Natives.functionCallCall(preLoopAttrs, this, attrsKeys);
					_shared.Natives.functionCallCall(preLoopAttrs, this, attrsSymbols);
					_shared.Natives.functionCallCall(loopAttrs, this, attrsKeys);
					_shared.Natives.functionCallCall(loopAttrs, this, attrsSymbols);
			...
			__Internal__._superFrom = root.DD_DOC(
				...
				, doodad.PROTECTED(doodad.READ_ONLY(doodad.CAN_BE_DESTROYED(doodad.PERSISTENT(doodad.TYPE(doodad.INSTANCE(doodad.JS_METHOD(
					function superFrom(cls) {
						...
						const newDispatch = types.getAttribute(proto, name, _shared.SECRET),
							notReentrantMap = __Internal__.notReentrantMap.get(this),
							self = this; // NOTE: That prevents to do "return _shared.Natives.functionBindCall(_superFrom, this)"
						...
						return function _superFrom(/*paramarray*/...params) {
							...
							try {
								return _shared.Natives.functionApplyCall(newDispatch, self, params);
							} finally {
								if (oldCalled) {
									notReentrantMap.set(name, oldCalled);
								};
							}
						};
					}))))))));
			...
			__Internal__._superAsync = root.DD_DOC(
					...
					function _superAsync() {
						...
						return _shared.Natives.functionBindCall(function superAsync(/*paramarray* /...params) {
							...
							const oldValues = types.getAttributes(this, ['_super', _shared.CurrentDispatchSymbol, _shared.CurrentCallerIndexSymbol], _shared.SECRET);
							...
							try {
								...
								types.setAttributes(this, attrs, _shared.SECRET);

								_super[_shared.CalledSymbol] = false;

								retVal = _shared.Natives.functionApplyCall(_super, this, params);
								...
								types.setAttributes(this, oldValues, _shared.SECRET);
							};
...
*/
			...
			__Internal__.callOutsideFn = function callOutside(fn, /*optional*/args) {
				const isInside = __Internal__.isInside(this);
				const oldValues = isInside && types.getAttributes(this, [_shared.CurrentDispatchSymbol, _shared.CurrentCallerIndexSymbol, '_super'], _shared.SECRET);
			...
			doodad.ADD('OutsideCallback', root.DD_DOC(
				...
				//! END_REPLACE()
				, function OutsideCallback(obj, fn) {
					if (types.isCallback(fn)) {
						throw new types.ValueError("The function is already a Callback.");
					};
					...
					types.setJsAttribute(cb, _shared.CallbackSymbol, doodad.OutsideCallback, {});
					types.setJsAttribute(cb, _shared.BoundObjectSymbol, obj, {});
					types.setJsAttribute(cb, _shared.OriginalValueSymbol, fn, {});
					return cb;
				}));

			types.setJsAttribute(doodad.OutsideCallback, _shared.BaseSymbol, doodad.Callback, {});
			...
			__Internal__.classProto = {
					...
					function _new(/*paramarray*/...args) {
						...
						if (forType) {
							...
							if (!isBase && !isStatic) {
								...
								types.setAttribute(this, _shared.InitInstanceSymbol, initInstance, _shared.SECRET);
							};

						} else {
							const instanceStorage = types.clone(types.getAttribute(cls.prototype, _shared.AttributesStorageSymbol, _shared.SECRET));
							//instanceStorage[_shared.AttributesStorageSymbol] = instanceStorage; // <<< Will be make by "_shared.InitInstanceSymbol"
				...
				toString: root.DD_DOC(
					...
					//! END_REPLACE()
					, doodad.PUBLIC(doodad.RETURNS(types.isString, doodad.PRE_EXTEND(doodad.TYPE(doodad.INSTANCE(
						function toString() {
				...
				getInterface: root.DD_DOC(
					...
					, doodad.PUBLIC(doodad.TYPE(doodad.INSTANCE(
						function getInterface(type) {
							...
							if (!cache) {
								cache = types.setAttribute(this, _shared.IsolatedCacheSymbol, new types.Map(), _shared.SECRET);
							};
							...
							const _implements = types.getAttribute(_interface, _shared.ImplementsSymbol, _shared.SECRET);
					...
					, doodad.PROTECTED(doodad.TYPE(doodad.INSTANCE(doodad.JS_METHOD(
						function restorePreserved(attr) {
							if (!types.isSymbol(attr)) {
								...
								if (extender && extender.isPreserved) {
									const preservedAttr = '__' + attr + '_preserved__';
									types.setAttribute(this, attr, this[preservedAttr], _shared.SECRET);
									return true;
								};
							};
							return false;
				...
				isImplemented: root.DD_DOC(
					...
					, doodad.PUBLIC(doodad.TYPE(doodad.INSTANCE(doodad.JS_METHOD(
						function isImplemented(name) {
							...
							const method = types.getAttribute(this, name, _shared.SECRET);
							return !((method[_shared.ModifiersSymbol] || 0) & doodad.MethodModifiers.NotImplemented);
						}))))),
			};
					...
					//! BEGIN_REMOVE()
						...
						if (customSymbol) {
							__Internal__.classProto[customSymbol] = doodad.PUBLIC(doodad.TYPE(doodad.INSTANCE(doodad.BIND(doodad.JS_METHOD(function inspect(depth, ctx) {
								const isType = types.isType(this),
									attrs = types.getAttribute(this, _shared.AttributesSymbol, _shared.SECRET) || {},
									result = {};
								const loopKeys = function _loopKeys(keys) {
										...
										if (key !== customSymbol) {
											...
											if (extender) {
												if ((attr[_shared.ScopeSymbol] === doodad.Scopes.Public) && ((isType && extender.isType) || (!isType && extender.isInstance))) {
													result[key] = types.getAttribute(this, key, _shared.SECRET);
												};
											};
										};
									};
								};
								_shared.Natives.functionCallCall(loopKeys, this, types.keys(attrs));
								_shared.Natives.functionCallCall(loopKeys, this, types.symbols(attrs));
								return result;
							})))));
						};
						//! BEGIN_REMOVE()
			...
			doodad.Class = doodad.BASE(_shared.Natives.functionCallCall(__Internal__.$extend, types.Type, __Internal__.classProto));
				...
				_new: types.SUPER(
					function _new(host) {
						...
						if (forType) {
							...
							types.setAttribute(this, _shared.HostSymbol, host, _shared.SECRET);
						};
			...
			doodad.Interface = doodad.BASE(_shared.Natives.functionCallCall(__Internal__.$extend, types.Type, __Internal__.interfaceProto));
			...
			root.DD_DOC(
				...
				//! END_REPLACE()
				, doodad.REGISTER(doodad.Interface));
			...
			root.DD_DOC(
				...
				, doodad.REGISTER(doodad.Class.$extend({
					...
					attach: root.DD_DOC(
						...
						//! END_REPLACE()
						, doodad.OUTSIDE(function attach(/*optional*/obj, fn, /*optional*/priority, /*optional*/datas, /*optional*/count) {
							const eventInside = __Internal__.setInside(this, _shared.SECRET, true);
							...
							if (indexesLen) {
								...
								let cb = fn;
								if (obj) {
									if ((obj === eventObj) || __Internal__.isInside(obj, eventInside)) {
					...
					attachOnce: root.DD_DOC(
						...
						//! END_REPLACE()
						, doodad.OUTSIDE(function attachOnce(obj, fn, /*optional*/priority, /*optional*/datas) {
				...
				, extenders.REGISTER([], extenders.Method.$inherit({
					...
					_new: types.SUPER(function _new(/*optional*/options) {
						...
						types.setAttributes(this, {
							...
							methodProperties: types.freezeObject(tools.append([_shared.ObjectSymbol, _shared.ExtenderSymbol, _shared.StackSymbol, _shared.SortedSymbol, _shared.ClonedStackSymbol], this.methodProperties)),
						});
					}),
					...
					extend: types.SUPER(function extend(attr, source, sourceProto, destAttributes, forType, sourceAttribute, destAttribute, sourceIsProto, proto, protoName) {
						if (root.getOptions().debug) {
							...
							if (this.prefix) {
								if (attr.slice(0, this.prefix.length) !== this.prefix) {
									throw new types.Error("The event named '~0~' must be prefixed by '~1~.", [attr, this.prefix]);
								};
							};
						};
					...
					remove: function remove(attr, obj, storage, forType, attribute) {
						...
						//_shared.Natives.functionCallCall(extenders.Attribute.remove, this, attr, obj, storage, forType, attribute);
					},
				})));
			...
			__Internal__.EVENT = function EVENT(/*optional*/cancellable, /*optional*/eventTypeName, /*optional*/fn) {
				...
				if (types.has(__Internal__.EVENT_CACHE, key)) {
					...
					eventFn = function handleEvent(/*optional*/ev) {
						...
						ev && types.setAttributes(ev, {obj: this, name: dispatch[_shared.NameSymbol]}, _shared.SECRET);
							...
							try {
								for (let i = 0; i < stackLen; i++) {
									...
									if (data[4] > 0) {
										...
										ev && types.setAttribute(ev, 'handlerData', data[3], _shared.SECRET);

										retval = _shared.Natives.functionCallCall(data[5], obj, ev);
									};
									...
									if ((retval === false) && cancellable) {
										...
										types.setAttributes(ev, {obj: this, name: this.onEventCancelled[_shared.NameSymbol]}, _shared.SECRET);
								...
								if (evObj) {
									types.setAttributes(ev, {obj: evObj, name: evName}, _shared.SECRET);
								};
							};
						};
			...
			__Internal__.RAW_EVENT = function RAW_EVENT(errorEvent, /*optional*/fn) {
				...
				if (types.has(__Internal__.RAW_EVENT_CACHE, key)) {
					...
					eventFn = function handleEvent(/*paramarray*/...params) {
						...
						const dispatch = types.getAttribute(this, _shared.CurrentDispatchSymbol, _shared.SECRET);
						...
						try {
							for (let i = 0; i < stackLen; i++) {
								...
								if (data[4] > 0) {
									...
									_shared.Natives.functionApplyCall(cb, obj, params);
			...
			doodad.ADD('EVENT', root.DD_DOC(
				...
				, function EVENT(/*optional*/cancellable, /*optional*/fn) {
					const attribute = __Internal__.EVENT(cancellable, 'Doodad.Event', fn);
					return attribute;
				}));
			...
			doodad.ADD('ERROR_EVENT', root.DD_DOC(
				...
				, function ERROR_EVENT(/*optional*/fn) {
					const attribute = __Internal__.EVENT(false, 'Doodad.ErrorEvent', fn);
					return attribute;
				}));
			...
			doodad.ADD('RAW_EVENT', root.DD_DOC(
				...
				, function RAW_EVENT(/*optional*/fn) {
					const attribute = __Internal__.RAW_EVENT(false, fn);
					return attribute;
				}));
			...
			doodad.ADD('RAW_ERROR_EVENT', root.DD_DOC(
				...
				, function RAW_ERROR_EVENT(/*optional*/fn) {
					const attribute = __Internal__.RAW_EVENT(true, fn);
					return attribute;
				}));
			...
			__Internal__.CANCEL_EVENT = root.DD_DOC(
				...
				, function CANCEL_EVENT(/*optional*/fn) {
					const attribute = __Internal__.EVENT(false, 'Doodad.CancelEvent', fn);
					return attribute;
				});
			...
			root.DD_DOC(
				...
				, interfaces.REGISTER(doodad.INTERFACE(doodad.Class.$extend({
					...
					[types.ClonableSymbol]: doodad.PUBLIC(doodad.READ_ONLY( true )),
			...
			__Internal__.creatablePrototype = {
				...
				[_shared.DestroyedSymbol]: doodad.PRIVATE(doodad.READ_ONLY(doodad.PERSISTENT(doodad.TYPE(doodad.INSTANCE(doodad.ATTRIBUTE(null, extenders.Attribute, {preInit: true, isProto: null})))))),
					...
					, doodad.PUBLIC(doodad.EXTERNAL(doodad.CALL_FIRST(
						function fastDestroy() {
							if (this[_shared.DestroyedSymbol] === false) {
								...
								_shared.Natives.functionCallCall(types.Type.prototype._delete, this);
							};
						})))),
			...
			root.DD_DOC(
				...
				, mixIns.REGISTER(doodad.MIX_IN(doodad.Class.$extend({
					...
					$__translations: doodad.PROTECTED(doodad.ATTRIBUTE(tools.nullObject(), extenders.ExtendObject, {maxDepth: 5, isProto: false, cloneOnInit: true})),
			...
			root.DD_DOC(
				...
				, mixIns.REGISTER(doodad.MIX_IN(doodad.Class.$extend({
					...
					$__config: doodad.PROTECTED(doodad.ATTRIBUTE(tools.nullObject(), extenders.ExtendObject, {maxDepth: 5, isProto: false, cloneOnInit: true})),
					...
					interfaces.Serializable,
					{
						...
						$pack: doodad.PUBLIC(doodad.TYPE(doodad.JS_METHOD(function $pack(value) {
							...
							if (types.isNothing(value)) {
								...
								_shared.Natives.functionCallCall(types.Error.prototype.parse, value);
								...
								if (type === 'undefined') {
									...
									tools.forEach(this.$ERROR_ATTRIBUTES, function(key) {
										if (types.has(value, key)) {
											types.setJsAttribute(tmp, key, this.$unpack(value[key]), {ignoreWhenReadOnly: true, configurable: true});
										};
									}, this);
									value = tmp;
								} else if (type === 'object') {
									...
									if (isError) {
										...
										value = this.$unpack(value);
										value = _shared.Natives.functionCallCall(fromJSON, cls, value);
									};
								} else {
